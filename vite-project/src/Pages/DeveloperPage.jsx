...
import ImportBookListItem from "../components/ImportBookListItem";
  ...
  async function retrieve_available_books() {
    try {
      ...
      if (response.status === 200 && Array.isArray(response.data)) {
        ...
        let sanitized = response.data.map(str => str.replace(/_/g, " "));
        sanitized = sanitized.map(str => str.replace(" .epub", ".epub"));
        setSanitizedBooksForImport(sanitized);
        console.log("Available books for import:", response.data);
        console.log("Sanitized books for import:", sanitized);
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="developer-page-main-container">
                ...
                {availableBooksForImport.length === 0 ? (
                  ...
                  availableBooksForImport.map((bookLabel, idx) => (
                    <ImportBookListItem
                      ...
                      bookLabel={bookLabel}
                      idx={idx}
                      displayLabel={sanitizedBooksForImport[idx]}
                      onImport={importBook}
                    />
                  ))
                )}
        ...
        <div className={"developer-page-center-panel"}>
          <div className={"developer-page-center-actions-panel"}>
            ...
            <div className="developer-page-action-cards-grid">
              ...
              <div className="developer-page-action-card">
                ...
                <button
                  className="button"
                  onClick={retrieve_imported_books}
                >View</button>
              </div>
              <div className="developer-page-action-card">
            ...
            <div className="developer-page-half-width-panel">
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
                {displayedBooks.map((book, idx) => (
                  <ImportBookCard key={book.bookID || idx} book={book} />))}
              </div>
            </div>
          </div>
        </div>
